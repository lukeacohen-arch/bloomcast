...
// Reads key injected by index.html in Vite; undefined in artifact sandbox (auth is automatic)
...
async function fetchINatPhoto(query) {
  ...
  try {
    const r = await fetch(
      `https://api.inaturalist.org/v1/taxa?q=${encodeURIComponent(query)}&per_page=1&is_active=true&rank=species`,
      { signal: ctrl.signal }
    );
    clearTimeout(timer);
    if (!r.ok) { photoCache[query] = null; return null; }
    const d = await r.json();
    const url = d.results?.[0]?.default_photo?.medium_url
             || d.results?.[0]?.default_photo?.square_url
             || null;
...
function usePhoto(query) {
  const [url,     setUrl]     = useState(photoCache[query] ?? null);
  const [loading, setLoading] = useState(photoCache[query] === undefined);
  useEffect(() => {
    if (!query) { setLoading(false); return; }
    if (photoCache[query] !== undefined) { setUrl(photoCache[query]); setLoading(false); return; }
    setLoading(true);
    fetchINatPhoto(query).then(u => { setUrl(u); setLoading(false); });
  }, [query]);
  return { url, loading };
}
    ...
    <div style={{ width:size, height:size, borderRadius:10, flexShrink:0, overflow:'hidden',
      ...
      {loading
        ? <div style={{ width:'55%', height:'55%', borderRadius:'50%',
            background:`radial-gradient(${color}90,${color}20)`,
...
function Banner({ query, color, height = 120 }) {
  const { url, loading } = usePhoto(query);
  return (
    <div style={{ width:'100%', height, overflow:'hidden', background:color+'18',
      display:'flex', alignItems:'center', justifyContent:'center' }}>
      {loading
        ? <div style={{ width:44, height:44, borderRadius:'50%',
            background:`radial-gradient(${color}80,${color}15)`,
            animation:'pulse 1.4s ease-in-out infinite' }}/>
        : url
          ? <img src={url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }}/>
...
function SpeciesDetail({ sp, onBack }) {
  ...
  const st = getStatus(sp, NOW_M);
  const ss = ST[st] || ST.blooming;
  const ti = TYPES[sp.type] || TYPES.wildflower;
  ...
  const load = useCallback(() => {
    ...
    fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 900,
        system: "You are a botanist and beekeeping expert. Respond with a single raw JSON object only. No markdown. No code fences. No explanation. Begin your response with { and end with }.",
        messages: [{ role: "user", content:
          `Return a JSON profile for ${sp.name} (${sp.inat}) with exactly these keys: description (2-3 sentence botanical description), habitat (1-2 sentences on where it grows), beekeeping (2-3 sentences on nectar quality and hive impact), did_you_know (one surprising fact), peak_tip (one practical field tip).`
        }]
      })
    })
    .then(r => r.json())
    .then(d => {
      ...
      if (!parsed) try { parsed = JSON.parse(raw.replace(/```[\w]*/g, '').replace(/```/g, '').trim()); } catch (_) {}
      if (!parsed) { const m = raw.match(/\{[\s\S]*\}/); if (m) try { parsed = JSON.parse(m[0]); } catch (_) {} }
      if (parsed) { setInfo(parsed); setError(false); }
      else setError(true);
    })
    .catch(() => setError(true))
  ...
  const label = { fontSize:'10px', color:'#2d7a4a', fontWeight:'700', letterSpacing:'0.06em', marginBottom:'6px' };

  return (
    <div style={{ background:'#060f0a', minHeight:'100vh', color:'#c9e8d4', fontFamily:'system-ui,sans-serif' }}>
      {/* Back bar */}
      <div style={{ background:'#0a1f14', borderBottom:'1px solid #1a4a2e', padding:'12px 16px',
        display:'flex', alignItems:'center', gap:'12px' }}>
        <button onClick={onBack} style={{ background:'#1a4a2e', border:'none', borderRadius:'8px',
          color:'#4ade80', padding:'6px 14px', cursor:'pointer', fontSize:'13px', fontWeight:'600' }}>
          ← Back
        </button>
        <div>
      ...
      <div style={{ maxWidth:'580px', margin:'0 auto', padding:'14px',
        ...
        <div style={{ borderRadius:'14px', overflow:'hidden', position:'relative' }}>
          ...
          <div style={{ position:'absolute', inset:0,
            background:'linear-gradient(to bottom,transparent 40%,#060f0aee 100%)' }}/>
          <div style={{ position:'absolute', bottom:'14px', left:'16px', right:'16px',
            display:'flex', justifyContent:'space-between', alignItems:'flex-end' }}>
            <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
              <div style={{ padding:'4px 10px', borderRadius:'6px', fontSize:'11px',
                background:ss.bg, color:ss.tc, fontWeight:'600' }}>{ss.label}</div>
              <div style={{ padding:'4px 10px', borderRadius:'6px', fontSize:'11px',
                background:'#0008', color:ti.c, fontWeight:'600' }}>{ti.label}</div>
            </div>
            <div style={{ padding:'4px 10px', borderRadius:'6px', fontSize:'11px',
              background:'#0008', color:fc, fontWeight:'700' }}>Nectar {sp.ns}/10</div>
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'8px' }}>
          {[
            [sp.ns + '/10', 'Nectar Score', fc],
            [sp.pm.length,  'Bloom Months', '#4ade80'],
            [sp.pm.map(m => MO[m-1][0]).join(''), 'Season', '#818cf8'],
          ].map(([v, l, c]) => (
        ...
        {/* Timeline */}
        <div style={card}>
          <div style={label}>BLOOM SEASON</div>
          <div style={{ display:'flex', gap:'3px', marginBottom:'6px' }}>
            {Array.from({ length:12 }, (_, i) => i + 1).map(m => (
              <div key={m} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:'3px' }}>
                <div style={{ width:'100%', height:'10px', borderRadius:'2px',
                  background: sp.pm.includes(m) ? sp.c : '#0d2014',
                  opacity: sp.pm.includes(m) ? (m === NOW_M ? 1 : 0.6) : 1,
                  outline: m === NOW_M ? '1px solid #f5a62360' : 'none' }}/>
                <div style={{ fontSize:'8px', color: m === NOW_M ? '#f5a623' : '#2d5a3d' }}>{MO_S[m-1]}</div>
              </div>
            ))}
          </div>
          <div style={{ fontSize:'11px', color:'#2d7a4a' }}>
            Peak: <span style={{ color:'#8ab8a0' }}>{sp.pm.map(m => MO[m-1]).join(', ')}</span>
          </div>
        </div>

        {/* Nectar bar */}
        <div style={card}>
          <div style={label}>NECTAR VALUE</div>
          <div style={{ background:'#060f0a', borderRadius:'99px', height:'8px', overflow:'hidden', marginBottom:'6px' }}>
            <div style={{ height:'100%', borderRadius:'99px', width:`${(sp.ns/10)*100}%`,
              background:`linear-gradient(90deg,${fc}60,${fc})`, transition:'width 1s ease' }}/>
          </div>
          <div style={{ fontSize:'11px', color:'#2d5a3d' }}>
            {sp.ns >= 9 ? 'Exceptional nectar producer — highly sought by bees'
             : sp.ns >= 7 ? 'Strong nectar source — reliable for hive production'
             : sp.ns >= 5 ? 'Moderate nectar value — good supplemental forage'
             : 'Lower nectar yield — valuable for pollen and diversity'}
          </div>
        </div>
          ...
          <div style={{ ...card, display:'flex', flexDirection:'column', gap:'14px' }}>
            {['DESCRIPTION','HABITAT','BEEKEEPING NOTES'].map(l => (
              <div key={l}>
                ...
                <div style={{ height:'12px', borderRadius:'4px', background:'#1a4a2e',
                  opacity:0.5, animation:'pulse 1.4s ease-in-out infinite', marginBottom:'6px' }}/>
                <div style={{ height:'12px', borderRadius:'4px', background:'#1a4a2e',
                  opacity:0.3, width:'70%', animation:'pulse 1.4s ease-in-out infinite' }}/>
              </div>
            ))}
          </div>
        )}
        ...
        {!loading && error && (
          <div style={{ ...card, textAlign:'center' }}>
            <div style={{ color:'#4a7c5e', fontSize:'13px', marginBottom:'12px' }}>
              Failed to load species details.
            </div>
            <button onClick={load} style={{ background:'#1a4a2e', border:'1px solid #2d7a4a',
              borderRadius:'8px', color:'#4ade80', padding:'8px 18px',
        ...
        {!loading && info && (
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <div style={card}>
              ...
              <p style={{ margin:0, fontSize:'13px', lineHeight:'1.7', color:'#a7d9ba' }}>{info.description}</p>
            </div>
            <div style={card}>
              <div style={label}>HABITAT</div>
              <p style={{ margin:0, fontSize:'13px', lineHeight:'1.7', color:'#a7d9ba' }}>{info.habitat}</p>
            </div>
            <div style={{ ...card, borderColor:'#2d7a4a', background:'#071a10' }}>
              <div style={{ ...label, color:'#f5a623' }}>🐝 BEEKEEPING NOTES</div>
              <p style={{ margin:0, fontSize:'13px', lineHeight:'1.7', color:'#c9e8d4' }}>{info.beekeeping}</p>
            </div>
            <div style={card}>
              <div style={label}>FIELD TIP</div>
              <p style={{ margin:0, fontSize:'13px', lineHeight:'1.7', color:'#a7d9ba' }}>{info.peak_tip}</p>
            </div>
            <div style={{ ...card, borderStyle:'dashed' }}>
              <div style={label}>DID YOU KNOW</div>
              <p style={{ margin:0, fontSize:'13px', lineHeight:'1.7', color:'#6ab890', fontStyle:'italic' }}>{info.did_you_know}</p>
            </div>
          </div>
        )}
      </div>
...
function BloomRadar({ month }) {
  ...
  return (
    <svg width="260" height="260" viewBox="0 0 260 260" style={{ display:'block', margin:'0 auto' }}>
      ...
      <circle cx="130" cy="130" r="9" fill="#f5a623" opacity="0.95"/>
      <text x="130" y="133.5" textAnchor="middle" fontSize="10" dominantBaseline="middle">🐝</text>
    </svg>
  );
}
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages:[{ role:"user", content:
            `Region "${loc}", April 4 2026. JSON with:
"blooming":[{name,inat,nectar(1-10),type("forage"|"wildflower"|"ornamental"|"garden"|"orchard"),color(hex),status("blooming"|"opening"|"fading")}]
"upcoming":[name strings]
5-9 blooming, 3-5 upcoming. Regional flora. JSON only.`
          }]
        })
      });
      const d = await res.json();
  ...
  const avgNec    = dBloom.length
    ? Math.round(dBloom.reduce((a,b) => a+(b.ns||b.nectar||0), 0) / dBloom.length * 10) / 10 : 0;
  ...
  const C = {
    ...
    inp:  { background:'#0d2014', border:'1px solid #2d7a4a', borderRadius:'8px',
            color:'#c9e8d4', padding:'8px 12px', fontSize:'14px', outline:'none', flex:1 },
    btn:  { background:'#f5a623', color:'#060f0a', border:'none', borderRadius:'8px',
            padding:'8px 16px', fontWeight:'700', cursor:'pointer', fontSize:'13px', whiteSpace:'nowrap' },
    tab:  a => ({ padding:'7px 12px', borderRadius:'6px', border:'none', cursor:'pointer', fontSize:'12px',
            background:a?'#1a4a2e':'transparent', color:a?'#4ade80':'#4a7c5e', fontWeight:a?'600':'400' }),
  };
    ...
    <div style={C.wrap}>
      ...
      <div style={C.hdr}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'10px' }}>
          ...
          <div style={{ fontSize:'11px', textAlign:'right', color:'#4a7c5e' }}>
            <div>Apr 4, 2026</div>
            <div style={{ color:'#2d6040' }}>Early Spring</div>
          </div>
        </div>
      ...
      <div style={{ display:'flex', gap:'2px', padding:'8px 12px', background:'#081510',
        borderBottom:'1px solid #1a4a2e', overflowX:'auto' }}>
        {[['dashboard','📊 Dashboard'],['map','🗺 Radar'],['calendar','📅 Calendar'],['species','🌸 Species']].map(([k,l]) => (
      ...
      <div style={{ padding:'14px', maxWidth:'580px', margin:'0 auto', display:'flex', flexDirection:'column', gap:'12px' }}>
        ...
        {tab === 'dashboard' && (
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'8px' }}>
              {[[locLoad?'…':dBloom.length,'Blooming Now','#4ade80'],
                [avgNec,'Nectar Index',flowColor],
                [locLoad?'…':dUpcoming.length,'Coming Soon','#818cf8']
              ].map(([v,l,c]) => (
                ...
                <div key={i} className="spc" onClick={() => setSelected(spObj)}
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'6px', marginBottom:'2px' }}>
                      <div style={{ fontSize:'13px', fontWeight:'600', color:'#c9e8d4',
                        overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{s.name}</div>
                      <div style={{ fontSize:'10px', padding:'2px 7px', borderRadius:'4px',
                        background:ss.bg, color:ss.tc, fontWeight:'500', flexShrink:0 }}>{ss.label}</div>
                    </div>
                    <div style={{ fontSize:'11px', color:ti.c, marginBottom:'4px' }}>{ti.label}</div>
                    <div style={{ fontSize:'11px', color:'#f5a623', letterSpacing:'1px' }}>
                      {'▰'.repeat(ns)}{'▱'.repeat(10-ns)}
                    </div>
                  </div>
            ...
            {!locLoad && dUpcoming.length > 0 && (
              <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                ...
                <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
                  {dUpcoming.map((s, i) => (
                    <div key={i} style={{ padding:'5px 10px', borderRadius:'8px', fontSize:'11px',
        ...
        {tab === 'map' && (
          <div style={C.card}>
            <div style={{ fontSize:'11px', color:'#2d7a4a', fontWeight:'700', marginBottom:'4px' }}>BLOOM RADAR</div>
            ...
            <div style={{ display:'flex', gap:'3px', flexWrap:'wrap', marginBottom:'14px' }}>
              {MO.map((m, i) => (
                <button key={i} onClick={() => setViewMonth(i+1)} style={{
                  padding:'3px 7px', borderRadius:'4px', border:'none', cursor:'pointer', fontSize:'11px',
                  background: viewMonth===i+1 ? '#f5a623' : '#0d2014',
                  color: viewMonth===i+1 ? '#060f0a' : '#4a7c5e',
                  fontWeight: viewMonth===i+1 ? '700' : '400' }}>{m}</button>
              ))}
            </div>
            <BloomRadar month={viewMonth}/>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'100px repeat(12,1fr)', gap:'2px', marginBottom:'6px' }}>
                    ...
                    {MO_S.map((m,i) => (
                      <div key={i} style={{ textAlign:'center', fontSize:'9px',
                        color:i+1===NOW_M?'#f5a623':'#2d7a4a', fontWeight:i+1===NOW_M?'700':'400' }}>{m}</div>
                    ))}
                  </div>
                  {SP.map(s => (
                    <div key={s.id} style={{ display:'grid', gridTemplateColumns:'100px repeat(12,1fr)',
                      gap:'2px', marginBottom:'2px', alignItems:'center' }}>
                      <div style={{ fontSize:'10px', color:'#6ab890', overflow:'hidden',
                        textOverflow:'ellipsis', whiteSpace:'nowrap', paddingRight:'4px' }}>{s.name}</div>
                      {Array.from({ length:12 }, (_, i) => i+1).map(m => (
                        <div key={m} style={{ height:'11px', borderRadius:'2px',
                          background: s.pm.includes(m) ? s.c : '#0d2014',
                          opacity: s.pm.includes(m) ? (m===NOW_M?1:0.55) : 1,
                          outline: m===NOW_M ? '1px solid #f5a62350' : 'none' }}/>
                      ))}
                    </div>
                  ))}
                  <div style={{ marginTop:'10px', display:'flex', alignItems:'center', gap:'6px', fontSize:'10px', color:'#f5a623' }}>
                    <div style={{ width:'14px', height:'3px', background:'#f5a623', borderRadius:'1px' }}/>
                    Current month
                  </div>
                </div>
              </div>
                ...
                {Array.from({ length:12 }, (_, i) => {
                  ...
                  return (
                    <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:'4px' }}>
                      <div style={{ width:'100%', borderRadius:'3px 3px 0 0', height:`${(sc/10)*100}%`, minHeight:'2px',
                        background: m===NOW_M ? fc : 'linear-gradient(180deg,#2d7a4a80,#1a4a2e)' }}/>
        ...
        {tab === 'species' && (
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <div style={{ display:'flex', gap:'5px', flexWrap:'wrap' }}>
              <button onClick={() => setTypeFilter('all')} style={{ padding:'4px 10px', borderRadius:'99px', border:'none',
                cursor:'pointer', fontSize:'11px', background:typeFilter==='all'?'#1a4a2e':'#0d2014',
                color:typeFilter==='all'?'#4ade80':'#4a7c5e' }}>All</button>
              {Object.entries(TYPES).map(([k,v]) => (
                <button key={k} onClick={() => setTypeFilter(k)} style={{ padding:'4px 10px', borderRadius:'99px',
                  border:'none', cursor:'pointer', fontSize:'11px',
                  background:typeFilter===k?v.c+'22':'#0d2014', color:typeFilter===k?v.c:'#4a7c5e' }}>{v.label}</button>
              ))}
            </div>
            {filtered.map(s => {
              ...
              return (
                <div key={s.id} className="spc" onClick={() => setSelected(s)}
                  ...
                  <div style={{ position:'relative', overflow:'hidden' }}>
                    ...
                    <div style={{ position:'absolute', inset:0,
                      background:'linear-gradient(to bottom,transparent 25%,#0a1f14f0 100%)', pointerEvents:'none' }}/>
                    <div style={{ position:'absolute', bottom:'10px', left:'12px', right:'12px',
                      display:'flex', justifyContent:'space-between', alignItems:'flex-end' }}>
                      <div>
                        <div style={{ fontSize:'15px', fontWeight:'700', color:'#e8f8f0',
                          textShadow:'0 1px 6px #000a' }}>{s.name}</div>
                        <div style={{ fontSize:'10px', color:'#94a3b8', fontStyle:'italic' }}>{s.inat}</div>
                      </div>
                      <div style={{ display:'flex', gap:'6px', alignItems:'center' }}>
                        <div style={{ padding:'3px 8px', borderRadius:'5px', fontSize:'10px',
                          background:ss.bg, color:ss.tc, fontWeight:'500' }}>{ss.label}</div>
                  ...
                  <div style={{ padding:'10px 12px' }}>
                    <div style={{ display:'flex', gap:'2px', marginBottom:'8px' }}>
                      {Array.from({ length:12 }, (_, i) => i+1).map(m => (
                        <div key={m} style={{ flex:1, height:'5px', borderRadius:'1px',
                          background: s.pm.includes(m) ? s.c : '#0d2014',
                    ...
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:'11px' }}>
                      ...
                      <span style={{ color:s.ns>=8?'#f5a623':s.ns>=6?'#a3e635':'#4a7c5e', fontWeight:'600' }}>
                        Nectar {s.ns}/10
                      </span>
                    </div>
                  </div>
      ...
      <div style={{ padding:'16px', textAlign:'center', fontSize:'10px', color:'#1a3a22',
        borderTop:'1px solid #0d2014', marginTop:'8px' }}>
