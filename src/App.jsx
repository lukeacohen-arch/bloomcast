...
async function fetchINatPhoto(q) {
  ...
  try {
    const r = await fetch(
      `https://api.inaturalist.org/v1/taxa?q=${encodeURIComponent(q)}&per_page=1&is_active=true&rank=species`,
      { signal: ctrl.signal }
    );
    clearTimeout(t);
...
function usePhoto(q) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchINatPhoto(q).then(u => { setUrl(u); setLoading(false); });
  }, [q]);
  return { url, loading };
}
...
function Thumb({ query, color, size = 52 }) {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:12, flexShrink:0, overflow:'hidden',
      ...
      {loading
        ? <div style={{ width:'50%', height:'50%', borderRadius:'50%', background:`${color}50`, animation:'pulse 1.6s ease-in-out infinite' }}/>
        : url
...
function Banner({ query, color, height = 140 }) {
  const { url, loading } = usePhoto(query);
  return (
    <div style={{ width:'100%', height, overflow:'hidden', background:`${color}14`,
      display:'flex', alignItems:'center', justifyContent:'center' }}>
      {loading
        ? <div style={{ width:40, height:40, borderRadius:'50%', background:`${color}40`, animation:'pulse 1.6s ease-in-out infinite' }}/>
        : url
          ? <img src={url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }}/>
...
const validateReport = (form) => {
  ...
  if (form.species?.length > 100) errors.push("Plant name too long (max 100 chars)");
  ...
  if (urlPattern.test(form.species || '') || urlPattern.test(form.note || ''))
    errors.push("Links are not allowed in submissions");
  return errors;
};

const checkRateLimit = () => {
  const key = 'bloomcast_last_submit';
  const cooldown = 60 * 1000;
  const last = localStorage.getItem(key);
  if (last && Date.now() - parseInt(last) < cooldown) {
    const secondsLeft = Math.ceil((cooldown - (Date.now() - parseInt(last))) / 1000);
        ...
        <div style={{ padding:'0 18px 18px', borderTop:`1px solid ${T.border}` }}>
          {loading && (
            <div style={{ paddingTop:16, display:'flex', flexDirection:'column', gap:8 }}>
              {[85,60,72].map((w,i) => (
                <div key={i} style={{ height:11, borderRadius:6, background:T.surf2, width:`${w}%`, animation:'pulse 1.6s ease-in-out infinite' }}/>
              ))}
            </div>
          )}
          {!loading && result && (
            <div style={{ paddingTop:14, display:'flex', flexDirection:'column', gap:10 }}>
                ...
                <div key={i} style={{ background:T.bg, borderRadius:12, padding:'12px 14px', border:`1px solid ${T.border}` }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                    ...
                    <div style={{ fontSize:11, padding:'2px 8px', borderRadius:99,
                      background:`${rc[v.rarity] || T.accent}18`, color:rc[v.rarity] || T.accent }}>{v.rarity}</div>
                  </div>
              ...
              {result.harvest_timing && (
                <div style={{ fontSize:12, color:T.amber, padding:'8px 12px', background:`${T.amber}10`, borderRadius:8, border:`1px solid ${T.amber}20` }}>
                  Harvest window: {result.harvest_timing}
                </div>
              )}
              <button onClick={predict} style={{ background:'none', border:`1px solid ${T.border2}`, borderRadius:8,
                color:T.muted, padding:'6px', cursor:'pointer', fontSize:12, fontFamily:'inherit' }}>
                Regenerate
              </button>
            </div>
          )}
          {!loading && !result && (
            <div style={{ paddingTop:14, textAlign:'center' }}>
              <button onClick={predict} style={{ background:T.amber, border:'none', borderRadius:10,
                color:'#fff', padding:'9px 24px', cursor:'pointer', fontSize:13, fontWeight:700, fontFamily:'inherit' }}>
                Predict My Honey
              </button>
            </div>
          )}
  ...
  const load = useCallback(() => {
    ...
    fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: "user", content:
          `JSON profile for ${sp.name} (${sp.inat}). Keys: description, habitat, beekeeping, did_you_know, peak_tip.`
        }]
      })
    })
    .then(r => r.json())
    ...
    <div style={{ background:T.bg, minHeight:'100vh', color:T.text, fontFamily:'system-ui,sans-serif' }}>
      <div style={{ background:T.surf, borderBottom:`1px solid ${T.border}`, padding:'12px 16px',
        ...
        <button onClick={onBack} style={{ background:T.surf2, border:`1px solid ${T.border2}`, borderRadius:10,
          color:T.accent, padding:'6px 14px', cursor:'pointer', fontSize:13, fontWeight:600, fontFamily:'inherit' }}>
          ← Back
        </button>
        <div>
      ...
      <div style={{ maxWidth:580, margin:'0 auto', padding:14, display:'flex', flexDirection:'column', gap:12 }}>
        ...
        <div style={card}>
          <div style={lbl}>Nectar Value</div>
          <div style={{ background:T.bg, borderRadius:99, height:6, overflow:'hidden', marginBottom:8 }}>
            <div style={{ height:'100%', borderRadius:99, width:`${(sp.ns/10)*100}%`,
              background:`linear-gradient(90deg,${fc}80,${fc})`, transition:'width 1s ease' }}/>
          </div>
        ...
        {!loading && error && (
          <div style={{ ...card, textAlign:'center' }}>
            ...
            <button onClick={load} style={{ background:T.surf2, border:`1px solid ${T.border2}`, borderRadius:10,
              color:T.accent, padding:'8px 18px', cursor:'pointer', fontSize:13, fontWeight:600, fontFamily:'inherit' }}>
              Try Again
            </button>
          </div>
        )}
...
async function fetchRealHeatPoints(lat, lng, species, month) {
  ...
  const fetchSp = async sp => {
    try {
      const r = await fetch(
        `https://api.inaturalist.org/v1/observations?taxon_name=${encodeURIComponent(sp.inat)}&lat=${lat}&lng=${lng}&radius=40&month=${month}&per_page=200&quality_grade=research&geo=true`
      );
      if (!r.ok) return [];
      const d = await r.json();
      const w = sp.ns / 10;
  ...
  const all = await Promise.all(blooming.map(fetchSp));
  return all.flat();
}
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (hive) {
          const icon = L.divIcon({ html:'<div style="font-size:22px;filter:drop-shadow(0 1px 4px rgba(0,0,0,0.6))">🐝</div>',
        ...
        if (cancelled) return;
        if (pts.length > 0) {
          L.heatLayer(pts, { radius:28, blur:12, maxZoom:17, minOpacity:0.6, max:0.6,
...
export default function App() {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role:"user", content:
            `Region "${loc}", ${NOW_DATE}. JSON: "blooming":[{name,inat,nectar(1-10),type,color(hex),status("blooming"|"opening"|"fading")}] "upcoming":[names] 5-9 blooming, 3-5 upcoming. JSON only.`
          }]
        })
      });
      const d = await res.json();
  ...
  const resolveToSP = s => SP.find(sp => sp.name.toLowerCase() === s.name?.toLowerCase())
    || { ...SP[2], ...s, c: s.color || s.c || T.accent, ns: s.nectar || 5 };
  ...
  const tabBtn = active => ({
    ...
    fontSize:12, fontFamily:'inherit', whiteSpace:'nowrap', transition:'all 0.15s',
      ...
      <div style={{ background:T.surf, borderBottom:`1px solid ${T.border}`, padding:'12px 16px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:4 }}>
            <button onClick={() => setShowHiveForm(v => !v)}
              style={{ background: hive ? `${T.amber}18` : 'transparent',
                border:`1px solid ${hive ? T.amber + '40' : T.border2}`,
                borderRadius:99, color: hive ? T.amber : T.muted,
                padding:'4px 12px', cursor:'pointer', fontSize:11, fontWeight:600, fontFamily:'inherit' }}>
        ...
        {showHiveForm && (
          <div style={{ display:'flex', gap:8, marginBottom:10 }}>
            ...
            <button style={{ background:T.amber, color:'#fff', border:'none', borderRadius:10,
              padding:'8px 14px', fontWeight:700, cursor:'pointer', fontSize:13, fontFamily:'inherit' }}
              onClick={saveHive}>Save</button>
          </div>
        )}
        <div style={{ display:'flex', gap:8 }}>
          ...
          <button style={{ background:T.accent, color:'#fff', border:'none', borderRadius:12,
            ...
            onClick={handleSubmit}>Brief Me</button>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ padding:'10px 14px', background:T.surf, borderBottom:`1px solid ${T.border}`,
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div style={{ background:T.surf, borderRadius:20, padding:'22px 20px', border:`1px solid ${T.border}`, textAlign:'center' }}>
              ...
              <div style={{ fontSize:14, color:T.muted, marginBottom:16 }}>
                Nectar Flow Index · <span style={{ color:flowColor, fontWeight:600 }}>{flowLabel}</span>
              </div>
            ...
            {!locLoad && dBloom.map((s, i) => {
              ...
              return (
                <div key={i} className="spc" onClick={() => setSelected(spObj)}
                  style={{ background:T.surf, border:`1px solid ${T.border}`, borderRadius:16,
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:'flex', gap:2 }}>
                      {Array.from({ length:10 }, (_, j) => (
                        <div key={j} style={{ flex:1, height:3, borderRadius:99, background: j < ns ? tc : T.surf2 }}/>
                      ))}
                    </div>
                  </div>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {dUpcoming.map((s, i) => (
                    <div key={i} style={{ padding:'5px 12px', borderRadius:99, fontSize:12,
                      background:`${'#7c3aed'}14`, color:'#7c3aed', border:`1px solid ${'#7c3aed'}25` }}>
              ...
              <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                {MO.map((m, i) => (
                  <button key={i} onClick={() => setViewMonth(i + 1)} style={{
                    padding:'4px 9px', borderRadius:99, border:'none', cursor:'pointer', fontSize:11, fontFamily:'inherit',
                    background: viewMonth===i+1 ? T.accent : 'transparent',
                    color:      viewMonth===i+1 ? '#fff'   : T.muted,
                ...
                {Array.from({ length:12 }, (_, i) => {
                  ...
                  return (
                    <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                      <div style={{ width:'100%', borderRadius:'4px 4px 0 0', height:`${(sc/10)*100}%`, minHeight:2,
                        background: m === NOW_M ? fc : T.surf2 }}/>
            ...
            <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
              ...
              {Object.entries(TYPES).map(([k, v]) => (
                <button key={k} onClick={() => setTypeFilter(k)} style={{
                  padding:'6px 14px', borderRadius:99, border:'none', cursor:'pointer', fontSize:12, fontFamily:'inherit',
                  background: typeFilter===k ? `${v.c}18` : 'transparent',
                  color: typeFilter===k ? v.c : T.muted,
                  fontWeight: typeFilter===k ? 600 : 400 }}>{v.label}</button>
              ))}
            </div>
            {filtered.map(s => {
